...
export const COMPOSE_SYSTEM_INSTRUCTION_FASHION = [
  ...
  '',
  'ANALYZE BEFORE YOU RENDER (reason step by step):',
  "1. Locate the SUBJECT's EXISTING free arm (the one that is free — e.g. on a mirror selfie, the hand/arm NOT",
  '   holding the phone) and decide how this accessory is naturally carried on THAT arm — a handbag hangs from',
  '   the hand by its handle/strap, OR the handle loops',
  '   over the forearm and the bag rests in the crook of the elbow, whichever fits the handle and the pose.',
  ...
  "  subject's existing hand/forearm; nothing else may change.",
  '- USE THE EXISTING ARM — NEVER ADD A LIMB: carry the accessory on the arm the SUBJECT ALREADY has free (e.g.',
  '  on a mirror selfie, the one not holding the phone). NEVER add, duplicate, invent, or redraw a hand, arm,',
  '  finger, or limb — use only the arms the subject already has. If the free hand is not in a gripping pose,',
  '  HANG or rest the accessory on that existing hand/forearm/elbow rather than creating a new arm to hold it.',
  ...
  "- CONTACT/OCCLUSION: where the existing hand grips the handle, the subject's fingers and thumb wrap OVER it;",
  '  where the accessory hangs from the forearm or the crook of the elbow, the arm passes through the handle and',
  '  the bag rests against the body. It is carried by the existing arm — never floating in front of it, never',
  '  fused into the clothing.',
  ...
  "AVOID: altering the person's face or identity, changing the body or pose, ADDING A THIRD ARM OR HAND or any",
  ' extra/duplicated limb, a new hand created just to hold the accessory, a floating accessory, an accessory',
  ' fused into the clothing, a semi-transparent / translucent / see-through accessory, fingers clipping through',
  ' or merging into the handle instead of wrapping over it, wrong scale (too large or small for the hand),',
  ' duplicated accessory, changed product color,',
