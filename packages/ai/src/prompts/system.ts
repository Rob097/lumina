...
export const COMPOSE_SYSTEM_INSTRUCTION_FASHION = [
  ...
  '',
  'ANALYZE BEFORE YOU RENDER (reason step by step):',
  "1. Locate the SUBJECT's EXISTING free arm (the hand/arm NOT holding the phone) and decide how this accessory",
  '   is naturally carried on THAT arm — a handbag hangs from the hand by its handle/strap, OR the handle loops',
  ...
  '- USE THE EXISTING ARM — NEVER ADD A LIMB: carry the accessory on the arm the SUBJECT ALREADY has free (the',
  '  one not holding the phone). NEVER add, duplicate, invent, or redraw a hand, arm, finger, or limb — the',
  '  subject has exactly the two arms shown. If the free hand is not in a gripping pose, HANG or rest the',
  '  accessory on that existing hand/forearm/elbow rather than creating a new arm to hold it.',
