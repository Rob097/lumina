...
export const COMPOSE_SYSTEM_INSTRUCTION_FASHION = [
  ...
  '',
  'ANALYZE BEFORE YOU RENDER (reason step by step):',
  '1. Pick ONE arm to carry the accessory on. Choose the arm the SUBJECT has PREPARED to receive it — a hand',
  '   raised or curled as if gripping a handle, or a forearm bent and held out ready. Read this from the',
  "   SUBJECT's own photo: it is the strongest, most reliable cue and OVERRIDES the guide (left/right copied",
  '   from a guide/mirror selfie is unreliable). Only if NEITHER arm is clearly prepared, use the arm NOT',
  '   holding the phone; if both hands hold the phone, choose ONE forearm. Then decide how the accessory is',
  '   carried on THAT single arm — a handbag hangs from the hand by its handle/strap, OR the handle loops over',
  '   the forearm and the bag rests in the crook of the elbow, whichever fits the handle and the pose.',
  '2. Determine the correct SCALE. The given real-world dimensions are the AUTHORITY for size — render the',
  '   accessory exactly that big against the body. Anchor to the body: an adult hand is about 18 cm long and',
  '   8-9 cm wide and a forearm about 25 cm, so a small handbag (e.g. 20x10 cm) is only about the size of ONE',
  '   hand — keep it small. NEVER enlarge it to fill the torso or the frame, and never size it to a room, door,',
  '   wall, or car.',
  ...
  '- EXACTLY ONE accessory, on ONE arm only. Insert a SINGLE bag carried on a SINGLE arm. NEVER place a second',
  '  bag on the other arm, never duplicate or mirror it — even if both arms look free or both hands hold the',
  '  phone, only ONE arm carries the one bag and the other arm stays exactly as in the original.',
  ...
  ' extra/duplicated limb, a new hand created just to hold the accessory, TWO bags or a bag on EACH arm, a',
  ' second or mirrored accessory, an OVERSIZED accessory bigger than the hand/forearm when its real size is',
  ' small, a floating accessory, an accessory fused into the clothing, a semi-transparent / translucent /',
  ' see-through accessory, fingers clipping through or merging into the handle instead of wrapping over it,',
  ' wrong scale (too large or small for the hand), duplicated accessory, changed product color,',
