...
export function buildPlannerPrompt(input: PlannerInput): string {
  ...
  lines.push(
    ...
    '- productAnalysis: a concise (1–2 sentence) description of the PRODUCT for the compositor — what it is,',
    '  its key visual identity (form, material, colour, finish, distinctive parts), how it is normally',
    '  installed or placed, and its typical real-world size if inferable. Describe the product itself, not the',
    "  product photo's background (ignore any backdrop/props in the product image).",
